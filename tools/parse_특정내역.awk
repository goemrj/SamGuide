...

{
  ...
  if (left ~ /^[A-Z][A-Z][0-9][0-9][0-9]/){
    ...
    # 이름이 끝난 뒤로는 **줄 전체**를 본문으로 쓴다. 3단 시작 위치부터 잘라 쓰면
    # 첫 단 자리까지 삐져나온 표(MT018 본인부담구분코드)가 잘리고, 더 나쁜 것은
    # 잘린 줄만 왼쪽으로 밀려 **줄마다 칸 좌표가 달라진다** — 칸 안에서 줄바꿈된 글자
    # (M013 의 "노숙인 1종")가 구분코드 칸 자리로 옮겨 붙는다.
    # 줄 전체를 쓰면 모든 줄이 같은 좌표를 쓰고, 공통 들여쓰기는 flush() 가 걷어낸다.
    right = rtrim(line)
