...
$kinds = [ordered]@{
  ...
  # 자보한방을 자보보다 **먼저** 둔다 — 자보 문서에는 청구서(H)와 의·치과 명세서(A~E)가 같이 있어서,
  # 순서가 반대면 자보한방의 A 를 자보 문서에서 찾아 버린다. 청구서(C010)는 둘이 함께 쓰므로
  # 자보한방에 H 가 없어 자보 문서로 넘어간다.
  '자보한방'        = @{ label = '자보 한방 명세서';                  claim = 'JABO_HAN'   }
  '자보'            = @{ label = '자보 명세서 (의 · 치과) · 청구서';   claim = 'JABO,JABO_HAN' }
