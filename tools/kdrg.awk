...
function kind(i,   m,f1,f2,w){
  ...
  # 오류 DRG 머리줄은 **숫자**다 — 960·961·962·963·990·999.
  # 질병군처럼 A00 꼴이 아니라 그냥 지나쳤는데, 그러면 그 뒤에 딸린 표가 **앞 질병군의 표를
  # 덮어쓴다.** MDC 14 에서 962「시술과 일치하지 않는 산과 주진단」의 「주진단명 table1」이
  # O642 의 「주진단명 table1」을 밀어내 산전 질환 진단이 통째로 사라졌다(2026-08-26).
  # 질병군으로 함께 내보내지만 부표2 에 없는 번호라 화면이 알아서 거른다.
  if(m>=2 && f1 ~ /^(96[0-3]|99[09])$/ && f2 ~ /[가-힣]/) return "DRG";
